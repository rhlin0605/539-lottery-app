# app.py
# Streamlit app for California Fantasy 5 (加州天天樂) auto-fetch + strategy simulation
...
SOURCE_SC888 = "https://sc888.net/index.php?s=%2FLotteryFan%2Findex"
...
def parse_sc888_fantasy5(html: str) -> pd.DataFrame:
    ...
    Regex scan for: YYYY-MM-DD + 5 numbers (1-39)
    Example:
...
def merge_and_update(local_df: pd.DataFrame, web_df: pd.DataFrame):
...
def get_top_hot_numbers(draws, top_n, exclude_recent):
...
def simulate_pair_hit(draws, pair, simulations, sample_size):
    hits = 0
...
def run_strategy(df, recent_n, top_n, exclude_recent, simulations, sample_size, seed):
    draws = df_to_draw_sets(df, recent_n=recent_n)
    if len(draws) < max(sample_size, exclude_recent, 10):
        raise ValueError(
            f"可用期數不足：目前只有 {len(draws)} 期，至少需要 >= {max(sample_size, exclude_recent, 10)} 期。"
        )
...
st.title("加州天天樂 (Fantasy 5)｜自動抓取最新號碼 + 策略分析")
st.caption("資料來源預設使用第三方結果頁。請理性看待模擬結果。")
...
with st.sidebar:
    ...
    seed = st.number_input("Random seed（固定可重現）", min_value=0, max_value=10000, value=66, step=1)
