...
def parse_date_with_fallback(date_str: str, current_year=2025, default_month=12):
    match = re.match(r"(\d{1,2})\(?[一二三四五六日]?\)?", date_str)
    if match:
        day = int(match.group(1))
        try:
            dt = datetime(current_year, default_month, day)
